...
export class VoyageController {
  ...
  async firstStep(req,res) {
    ...
    const mail   = await mailService.sendMailBienvenue({firstname : result.returnObject?.firstname ,lastname : result.returnObject?.lastname , phone : result.returnObject?.phone,email : result.returnObject?.email});
